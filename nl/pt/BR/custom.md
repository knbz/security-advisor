...
# Ferramentas de segurança customizadas
{: #setup_custom}

Há momentos em que já pode haver uma ferramenta que você usa. É possível integrar essas ferramentas, como o NeuVector, ao {{site.data.keyword.security-advisor_short}}.
{: shortdesc}
...
É possível integrar suas ferramentas de segurança usando o painel do {{site.data.keyword.security-advisor_short}}.
{: shortdesc}

**Antes de começar**

* Deve-se ter uma conta com o parceiro que você deseja integrar.
...
3. Clique em **Integrações customizadas** e, em seguida, clique no cartão **Incluir solução customizada**. Uma tela é exibida.
  1. Dê um nome à sua solução. É possível usar apenas caracteres alfanuméricos, espaços em branco e traços (-).
  ...
  3. Faça upload de um ícone ou de uma imagem para representar a ferramenta.
    ...
    O {{site.data.keyword.security-advisor_short}} cria os artefatos que são necessários para a integração, como o ID do serviço, a chave de API, o ID da conta e os metadados. A função `writer` é designada.

4. É possível configurar a conta do cliente na guia **Integração** do outro serviço.
5. Com a integração em vigor, é possível começar a postar ocorrências no {{site.data.keyword.security-advisor_short}} e visualizar as descobertas no painel de serviço.
