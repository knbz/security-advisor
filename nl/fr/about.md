...
* De Network Insights
* D'Activity Insights
* Des partenaires commerciaux d'IBM tels que NeuVector et Twistlock
...
L'API Findings de {{site.data.keyword.security-advisor_short}} suit la spécification de l'API des métadonnées d'artefact [Grafeas](https://grafeas.io/){: external} pour stocker, interroger et extraire les métadonnées critiques. Les résultats sont fournis par les outils et les services de sécurité.
...
Découvrez les différents concepts que vous pouvez utiliser lorsque vous travaillez avec {{site.data.keyword.security-advisor_short}}.
{: shortdesc}
...
<dl>
  ...
  <dt>Carte</dt>
    <dd>Les métadonnées utilisées pour visualiser les résultats dans le tableau de bord du service sont définies par le type de note <code>CARTE</code>. {{site.data.keyword.security-advisor_short}} prend en charge trois types d'éléments d'indicateur clé de risques pour une carte : <ul><li>Numérique</li><li>Répartition</li><li>Série temporelle</li></ul></dd>
  <dt>Fournisseur</dt>
  ...
  <dt>Nom de ressource de cloud du service</dt>
    <dd>Le nom de ressource de cloud (CRN) du service identifie le service {{site.data.keyword.cloud_notm}} impliqué dans le résultat. Par exemple, dans une constatation d'expiration de certificat, l'ID d'instance de service ou le nom de ressource de cloud de l'instance de service Certificate Manager qui notifie les résultats est inclus.</dd>
...
Actuellement, {{site.data.keyword.security-advisor_short}} est pris en charge dans les régions Dallas et Londres. Dans chacune des régions prises en charge, le service s'exécute dans plusieurs [zones de disponibilité](https://www.ibm.com/cloud/blog/announcements/improving-app-availability-multizone-clusters){: external}. Une reprise après incident régionale a été mise en place pour {{site.data.keyword.security-advisor_short}}. Le service gère une base de données de sauvegarde qui peut être restaurée en trois heures. Toutes les données du service, sauf celles des dernières vingt-quatre heures, sont conservées. 
